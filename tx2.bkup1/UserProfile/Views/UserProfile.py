...
from tx2.UserProfile.models import StudentDetails, Degree,Branch,Category
from tx2.Misc.MIscFunctions1 import is_integer
import logging

# to be removed after useremail gets placed in session
...
def UserHomeIndex(HttpRequest):
    ...
    return render_to_response("UserProfile/UserHome.html",{'title':"User Home",'LoggedUser':LoggedUser,'BranchList':Branch.objects.all(),'CategoryList':Category.objects.all(),'DegreeList':Degree.objects.all()},context_instance=RequestContext(HttpRequest))
...
def StudentDetailsIndex(HttpRequest):
    ...
    return render_to_response("UserProfile/StudentDetails.html",{'StudDetailStatus':StudDetailStatus,'Logged_in':Logged_in,'BranchList':Branch.objects.all(),'CategoryList':Category.objects.all(),'DegreeList':Degree.objects.all()},context_instance=RequestContext(HttpRequest))
...
def StudentDetailsInsert(HttpRequest):
    ...
    if( logindetails["userid"] == -1):
            ...
            HttpRequest.session[SESSION_MESSAGE] = msglist
            Logged_in=False
            return render_to_response("UserProfile/StudentDetails.html",{'Logged_in':Logged_in},context_instance=RequestContext(HttpRequest))
